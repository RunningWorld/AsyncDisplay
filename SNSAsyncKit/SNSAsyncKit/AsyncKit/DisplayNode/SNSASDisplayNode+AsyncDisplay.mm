//
//  SNSASDisplayNode+AsyncDisplay.m
//  SNSAsyncKit
//
...
@interface SNSASDisplayNode () <SNSASDisplayLayerDelegate>
@end

@implementation SNSASDisplayNode (AsyncDisplay)
...
- (void)_recursivelyRasterizeSelfAndSublayersWithIsCancelledBlock:(asdisplaynode_iscancelled_block_t)isCancelledBlock displayBlocks:(NSMutableArray *)displayBlocks
{
    ...
    for (SNSASDisplayNode *subnode in self.subnodes) {
...
- (void)displayAsyncLayer:(SNSASDisplayLayer *)asyncLayer asynchronously:(BOOL)asynchronously
{
    ...
    if (asynchronously) {
        ...
        __weak SNSASDisplayNode *weakSelf = self;
        isCancelledBlock = ^BOOL{
            __strong SNSASDisplayNode *self = weakSelf;
...
//                ASDisplayNodeSetResizableContents(layer, image);
            ...
            // TODO: rasterizesSubtree
            if (rasterizesSubtree) {
//                ASDisplayNodePerformBlockOnEverySubnode(self, NO, ^(SNSASDisplayNode * _Nonnull node) {
...
//        });
    ...
    if (asynchronously) {
        ...
        SNSASTransaction *transaction = containerLayer.asyncdisplaykit_asyncTransaction;
        ...
        [transaction addOperationWithBlock:displayBlock priority:self.drawingPriority queue:[SNSASDisplayLayer displayQueue] completion:completionBlock];
    } else {
