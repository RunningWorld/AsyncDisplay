...
@interface SNSASTransaction ()
@property ASAsyncTransactionState state;
@end

@implementation SNSASTransaction
{
...
- (instancetype)initWithCompletionBlock:(void(^)(SNSASTransaction *, BOOL))completionBlock
{
    if ((self = [self init])) {
    ...
    if (self.state != ASAsyncTransactionStateComplete) {
        if (_group) {
            ...
            if (self.state == ASAsyncTransactionStateOpen) {
                [SNSASTransactionGroup.mainTransactionGroup commit];
